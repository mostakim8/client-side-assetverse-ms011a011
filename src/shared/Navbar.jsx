        ...
        <>
            ...
            {user && role === 'hr' && (
                <>
                    ...
                    <li onClick={() => setIsOpen(false)}><NavLink to="/upgrade" className={linkStyle}>Upgrade</NavLink></li>
                </>
            )}
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 bg-white border-b border-gray-200 z-50 h-16 flex items-center shadow-sm">
            <div className="max-w-7xl mx-auto w-full px-4 flex items-center justify-between relative">
                ...
                {isOpen && (
                    <div className="absolute top-16 left-0 w-full bg-white shadow-xl border-b border-gray-100 lg:hidden z-40">
                        <ul className="flex flex-col p-4 space-y-2 list-none m-0">
                            {menuItems}
                            {user && (
                                <li className="pt-2">
                                    <button onClick={handleLogOut} className="w-full btn btn-error btn-sm text-white">Logout</button>
                                </li>
                            )}
                        </ul>
                    </div>
