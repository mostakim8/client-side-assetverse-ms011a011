...
const Navbar = () => {
    ...
    const [role] = UseRole();
    const [isOpen, setIsOpen] = useState(false); // মেনু খোলা বা বন্ধ রাখার স্টেট
    ...
    const menuItems = (
        <>
            <li onClick={() => setIsOpen(false)}><NavLink to="/" className={({ isActive }) => isActive ? activeStyle : linkStyle}>Home</NavLink></li>
            {!user && (
                <>
            ...
            {user && role === 'hr' && (
                <>
                    ...
                    <li onClick={() => setIsOpen(false)}><NavLink to="/my-employee-list" className={linkStyle}>Employee List</NavLink></li>
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 bg-white border-b border-gray-200 z-50 h-16 flex items-center shadow-sm">
            <div className="max-w-7xl mx-auto w-full px-4 flex items-center justify-between relative">
                ...
                {/* ১. লোগো এবং মোবাইল টগল বাটন */}
                <div className="flex items-center">
                    <button 
                        onClick={() => setIsOpen(!isOpen)} // ক্লিক করলে উল্টোটা হবে (Toggle)
                ...
                {/* ২. মোবাইল ড্রপডাউন মেনু (স্টেটের ওপর ভিত্তি করে শো/হাইড হবে) */}
                {isOpen && (
                    <div className="absolute top-12 left-0 w-full bg-white shadow-xl border-b border-gray-100 lg:hidden animate-in slide-in-from-top duration-300 z-40">
                ...
                {/* ৩. বড় ডিসপ্লের জন্য সরাসরি মেনু (সবসময় থাকবে) */}
                    ...
                    {user ? (
                        <div className="flex items-center gap-3">
                            <div className="hidden md:block text-right leading-none">
                                ...
                                <span className="text-[9px] uppercase text-blue-500 font-black">{role}</span>
                            </div>
                            <img 
